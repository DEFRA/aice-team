...
This page outlines how a review of JavaScript code should be conducted within the Defra AICE team — what to flag, at what severity, and how to phrase it. It does not say what the code itself should look like; for that, see the [AICE JavaScript Style Guide](./javascript.md) and the [AICE JavaScript Testing Standards](./javascript-testing.md).

This guide applies whenever an AI reviewer — GitHub Copilot Reviews, or an agent using the AICE `javascript-review-standards` skill — reviews a JavaScript pull request for a Defra AICE project.
...
This guide governs *how* a review is conducted, not what the code should look like. When a comment flags a violation of an existing rule, cite the specific section of the [style guide](./javascript.md) or [testing standards](./javascript-testing.md) rather than restating the rule from memory — the guide is the source of truth, not the reviewer's recollection of it.
...
- Anything `neostandard`/ESLint already enforces or would autofix — see the style guide's [1.1 Linting / Formatting](./javascript.md#11-linting--formatting). Duplicating lint output as a review comment adds noise without adding information.
...
For anything that is not covered by these review standards, we recommend following the [AICE JavaScript Style Guide](./javascript.md), the [AICE JavaScript Testing Standards](./javascript-testing.md), and staying consistent with the existing review history. If alignment across AICE is required, please raise an issue in [Defra AICE Team GitHub](https://github.com/DEFRA/aice-team/issues).
